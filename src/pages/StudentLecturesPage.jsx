...
const useSignedUrl = (lectureId, type = "video", enabled = true) => {
  ...
  try {
    const endpoint = {
      pdf:      `${BASE_URL}/api/lectures/stream/pdf/${lectureId}`,
      video:    `${BASE_URL}/api/lectures/stream/video/${lectureId}`,
      excel:    `${BASE_URL}/api/lectures/stream/excel/${lectureId}`,
      document: `${BASE_URL}/api/lectures/stream/document/${lectureId}`,
      ppt:      `${BASE_URL}/api/lectures/stream/ppt/${lectureId}`,
    }[type];
...
// ✅ ADD SecureFileLink component HERE (before StudentLecturesPage)
const SecureFileLink = ({ lecture, fileType, icon, label, bgColor, borderColor, textColor }) => {
  const typeMap = { excel: "hasExcel", document: "hasDocument", ppt: "hasPpt" };
  const pathMap = { excel: "excelPath", document: "documentPath", ppt: "pptPath" };
  
  const hasFile = lecture[typeMap[fileType]] || lecture[pathMap[fileType]];
  const { signedUrl, urlLoading, urlError, refetch } = useSignedUrl(lecture._id, fileType, !!hasFile);

  if (!hasFile) return null;
  if (urlLoading) return <div style={{ padding: "12px 14px", fontSize: "13px", color: "#6b7280" }}>Loading {label}...</div>;
  if (urlError) return (
    <div style={{ padding: "12px 14px", color: "#991b1b", fontSize: "13px", background: "#fef2f2", borderRadius: "8px" }}>
      ⚠️ {urlError} <button onClick={refetch} style={{ background: "none", border: "none", color: "#3b82f6", cursor: "pointer" }}>Retry</button>
    </div>
  );
  if (!signedUrl) return null;
  ...
  return (
    <a href={signedUrl} target="_blank" rel="noopener noreferrer"
      style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 14px", background: bgColor, border: `1px solid ${borderColor}`, borderRadius: "8px", textDecoration: "none", color: textColor }}>
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        {icon}
        <div><div style={{ fontWeight: "600" }}>{label}</div><div style={{ fontSize: "11px" }}>Secure link — expires in 5 min</div></div>
      </div>
      <FaArrowRight />
    </a>
  );
...
const StudentLecturesPage = () => {
  ...
  if (loading) return (
    <div style={styles.page}>
      <Sidebar isMobile={isMobile} isOpen={isMobileSidebarOpen} onClose={() => setIsMobileSidebarOpen(false)} />
              ...
              <>
                ...
                {lectureHasResources && (
                  <div style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: "10px", padding: "14px" }}>
                    ...
                    {/* Replace these 3 static divs */}
{(selectedLecture.hasExcel || selectedLecture.excelPath) && (
  <SecureFileLink
    lecture={selectedLecture}
    fileType="excel"
    icon={<FaFileExcel style={{ fontSize: "18px" }} />}
    label="Exercise Files (Excel)"
...
{(selectedLecture.hasDocument || selectedLecture.documentPath) && (
  <SecureFileLink
    lecture={selectedLecture}
    fileType="document"
    icon={<FaFileWord style={{ fontSize: "18px" }} />}
    label="Word Document"
...
{(selectedLecture.hasPpt || selectedLecture.pptPath) && (
  <SecureFileLink
    lecture={selectedLecture}
    fileType="ppt"
    icon={<FaFilePowerpoint style={{ fontSize: "18px" }} />}
